...
def test(model, dataloader, output):
    ...
    df.to_csv(os.path.join("./data/results", output), index=False)
...
def main(test_file, pretrained_file, output, batch_size=32):
    """
    ...
    with open(test_file, "rb") as pkl:
        test_data = NLITestDataset(pickle.load(pkl))
    ...
    print(20 * "=", " Testing ESIM model on device: {} ".format(device),
          20 * "=")
    batch_time, total_time = test(model, test_loader, output)
...
if __name__ == "__main__":
 ...
 some dataset")
    parser.add_argument(
        "test_data", help="Path to a file containing preprocessed test data")
    ...
    main(args.test_data, args.checkpoint, args.batch_size)
